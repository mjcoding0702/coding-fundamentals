...
taskList.addEventListener('click', (e) => {
    if (e.target.classList.contains('delete-btn')){
        ...
        deleteTodo(id) //Make a copy to deleted Todos before remove from local storage
...
// Use ID to move between two columns
function deleteTodo(taskID){
    // Get selected item from todo list
    const todoItems = getTodoItems();
    const deletedTodo = todoItems.filter(todo => todo.id === taskID)
    console.log("deletedTodo")
    console.log(deletedTodo)
    const deletedTodoItem = {
        id: deletedTodo[0].id,
        item: deletedTodo[0].item,
        isPrioritized: deletedTodo[0].isPrioritized,
        isCompleted: deletedTodo[0].isCompleted
    }
    console.log(deletedTodoItem)

    let deletedTodoItems = [];
    const storedTodoItems = localStorage.getItem('deletedTodoItems');
    if (storedTodoItems) {
        deletedTodoItems = JSON.parse(storedTodoItems)
    }
    // Add the new todo item to the array
    deletedTodoItems.push(deletedTodoItem)

    // Update local storage with the updated todo items array
    localStorage.setItem('deletedTodoItems', JSON.stringify(deletedTodoItems));

    // Display the deleted Todo
    displayDeletedTodo()
}

function displayDeletedTodo() {
    const deletedTodoItems = getDeletedTodoItems()
    undoList.innerHTML = '';
    deletedTodoItems.forEach(deletedTodoItem => {
        const deletedtaskItem = `
            <div class="deleted-item" data-id="${deletedTodoItem.id}">
                <div class="task-content">
                    <p>${deletedTodoItem.item}</p>
                </div>
                <div class="task-button" >
        ...
        `
        undoList.innerHTML += deletedtaskItem;
    })
    return
}


function getDeletedTodoItems() {
    let deletedTodoItems = [];
    const storedTodoItems = localStorage.getItem('deletedTodoItems');
    if (storedTodoItems) {
        deletedTodoItems = JSON.parse(storedTodoItems)
...
undoList.addEventListener('click', (e) => {
    if (e.target.classList.contains('undo-btn')){
        const id = parseInt(e.target.closest('.deleted-item').dataset.id); //ID of the current delete button clicked. Need to parse from string to an integer
        // Get selected item from todo list
        ...
        const deletedTodo = deletedTodoItems.filter(item => item.id === id)
        console.log(deletedTodo)
        const newTodoItem = {
            id: deletedTodo[0].id,
            item: deletedTodo[0].item,
        ...
        //Find the index of the item with the given ID
        const index = deletedTodoItems.findIndex(deletedItem => deletedItem.id === id);

        // remove the item from the array
        deletedTodoItems.splice(index,1);

        // update local storage with the modified array
        localStorage.setItem('deletedTodoItems', JSON.stringify(deletedTodoItems));

        // remove item from the DOM
        e.target.closest('.deleted-item').remove(); //Targets the parent of the selected delete button
    }
})
...
undoList.addEventListener('click', (e) => {
    if (e.target.classList.contains('delete-btn')){
        ...
        //Find the index of the item with the given ID
        const index = deletedTodoItems.findIndex(item => item.id === id);
